...
void ESPRandom::uuid1(uint8_t* buffer) {
  ...
  uint8_t mac_address[6]; // Create MAC array
...
bool ESPRandom::isValidV4Uuid(String& uuid) {
  return isValidV4Uuid(static_cast<const uint8_t*>(uuid.c_str()))
}
...
uint16_t ESPRandom::clock_sequence_ = 0;